...
import {searchInContent} from "./searchInContent.js";
import {searchInTitle} from "./searchInTitle.js";
...
export async function searchBM25(serviceCanisterId, query) {
  ...
  console.log("1")
  let hosts_content = await searchInContent(serviceActor, query);
  console.log("2")
  let hosts_title = await searchInTitle(serviceActor, query);
  console.log("3")


  const weight = 0.5;
  // if there are same canister id between title and content results, sum the score and delete it in title results.
